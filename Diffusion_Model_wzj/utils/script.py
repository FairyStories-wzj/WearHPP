...
def create_model_and_diffusion(cfg):
    ...
    model = MotionTransformer(
        # input_feats=3 * cfg.joint_num,  # 3 means x, y, z  # 修改：三维改二维
        #  TODO: 这里要三维改成二维的
        input_feats = 2 * cfg.joint_num,
    ...
    diffusion = Diffusion(
        ...
        # motion_size=(cfg.n_pre, 3 * cfg.joint_num),  # 3 means x, y, z # 修改：三维改二维
        #  TODO: 这里要三维改成二维的
        motion_size = (cfg.n_pre, 2 * cfg.joint_num),
...
def get_multimodal_gt_full(logger, dataset_multi_test, args, cfg):
    ...
    # data_group: numpy数组，(samples, frame, joint, 3)，其中samples是采样的总数
    all_data = data_group[..., 1:, :].reshape(data_group.shape[0], data_group.shape[1], -1)
    # all_data: numpy张量（不是pytorch张量！），(samples, frame, (joint - 1) * 3)，去掉0号关节并将后两个维度合并
    gt_group = all_data[:, cfg.t_his:, :]
    # gt_group: numpy张量，(samples, frame - t_his, (joint - 1) * 3)，all_data中只保留预测的部分
    # gt_group = all_data[:, cfg.t_his:, :]  # 原程序中这一句话写了两遍
    ...
    logger.info('done...')
    logger.info('=' * 80)
    return {'traj_gt_arr': traj_gt_arr,  # 不开多模态这个返回一个None就可以了
...
def sample_preprocessing(traj, cfg, mode):
    ...
    elif mode == 'gif' or mode == 'gif_without_low_confs':
        n = cfg.vis_col
        traj = traj.repeat(n, 1, 1)
