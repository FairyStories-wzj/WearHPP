...
def post_process(pred, cfg):
    # pred = pred.reshape(pred.shape[0], pred.shape[1], -1, 3)  修改：三维改二维
    # pred = np.concatenate((np.tile(np.zeros((1, cfg.t_his + cfg.t_pred, 1, 3)), (pred.shape[0], 1, 1, 1)), pred),
    #                       axis=2)
    # print(pred.shape)
    pred = pred.reshape(pred.shape[0], pred.shape[1], -1, 2)
    # pred = np.concatenate((np.tile(np.zeros((1, cfg.t_his + cfg.t_pred, 1, 2)), (pred.shape[0], 1, 1, 1)), pred),
    #                                              axis=2)
    # pred[..., :1, :] = 0
    return pred
