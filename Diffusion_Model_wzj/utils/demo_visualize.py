...
def demo_visualize(mode, cfg, model, diffusion, dataset):
    ...
    if mode == 'switch':
        ...
        pose_gen = pose_generator(dataset['test'], model, diffusion, cfg, mode='gif')
        render_animation(dataset['test'].skeleton, pose_gen, ['HumanMAC'], cfg.t_his, ncol=4,
                         output=os.path.join(cfg.gif_dir, f'visualization.gif'))
